...
// ---- LVGL draw buffer ----
static lv_color_t buf1[SCREEN_W * 40];   // ~40 lines; increase if you want
static lv_disp_draw_buf_t draw_buf;

// ---- Touch calibration/orientation ----
static int16_t rxMin = 300,  rxMax = 3800;
static int16_t ryMin = 300,  ryMax = 3800;
static uint8_t TS_ROT = 0;          // start with 0 for portrait; try 1/2/3 if axes crossed
static bool    FLIP_X = false;      // set true if right/left mirrored AFTER TS_ROT is correct
static bool    FLIP_Y = false;      // set true if down/up mirrored AFTER TS_ROT is correct
...
static void my_disp_flush(lv_disp_drv_t *disp, const lv_area_t *area, lv_color_t *color_p) {
  ...
  // If LVGL is NOT swapping bytes (LV_COLOR_16_SWAP == 0), ask TFT_eSPI to swap on push.
#if LV_COLOR_16_SWAP
...
static void my_touch_read(lv_indev_drv_t * indev, lv_indev_data_t * data) {
  ...
  TS_Point p = ts.getPoint();   // already rotated by TS_ROT
  int16_t sx, sy;
...
void setup() {
  ...
  // TFT
  tft.init();
  tft.setRotation(0);          // portrait; use 2 if upside-down
