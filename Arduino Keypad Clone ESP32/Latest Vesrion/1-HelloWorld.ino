...
// If your SquareLine IDs use "Ideal Temp", map them to the generic Setpoint ones:
#ifndef UI_HAS_SETPOINT
#define UI_HAS_SETPOINT
#endif
#ifndef ui_Setpoint_Slider
#define ui_Setpoint_Slider ui_Ideal_Temp_Slider
#endif
#ifndef ui_Setpoint_Value
#define ui_Setpoint_Value  ui_Ideal_Temp_lable   // (sic) if your label is named like that
#endif

/***********************
 *  WIFI + MQTT CONFIG
 ***********************/
// Compiled-in defaults (used on first boot; then NVS takes over)
static const char* WIFI_SSID_DEF     = "Reaver";
static const char* WIFI_PASSWORD_DEF = "Figjam";

static const char* AP_SSID       = "HRV-Keypad";
static const char* AP_PASS       = "12345678";  // 8+ chars required by ESP

// MQTT defaults (first boot)
...
// Mutable (runtime/NVS) WiFi + MQTT settings
String   sta_ssid = WIFI_SSID_DEF;
String   sta_pass = WIFI_PASSWORD_DEF;
...
static int   target_percent      = 30;  // target holder
...
static bool  auto_mode = true;                    // start in AUTO
...
<h2>HRV Keypad</h2>
<div class=row>
  IP: <span id=ip class=val></span> | RSSI: <span id=rssi class=val></span> dBm | WiFi: <span id=wf class=val></span> |
  SSID: <span id=ssid class=val></span> | MAC: <span id=mac class=val></span>
</div>
...
<details class=row>
  <summary><b>Wi-Fi Settings</b></summary>
  <fieldset>
  ...
  <div class=row>
    <button class=btn id=wfSave>Save & Reconnect</button>
    <span id=wfMsg></span>
  </div>
...
async function loadWiFi(){
  try{
    const w=await q('/wifi');
    g('wfssid').value=w.ssid||'';
    g('wfpw').value=w.pass||'';
  }catch(e){}
}
...
async function saveWiFi(){
  ...
  try{
    const r=await q(`/wifi?ssid=${ssid}&pass=${pass}`);
    g('wfMsg').textContent=r.ok?'Saved. Reconnecting…':'Failed';
    setTimeout(()=>g('wfMsg').textContent='',2500);
  }catch(e){g('wfMsg').textContent='Error';}
}
g('mqttSave').onclick=saveMQTT;
g('wfSave').onclick=saveWiFi;
...
async function tick(){
  try{
    ...
    const br=s.boost_remaining_s||0;
    const bt=s.boost_total_s||300;
    if (s.boost && br>0){
      const m=Math.floor(br/60), sec=br%60;
      const mm=(m<10?'0':'')+m, ss=(sec<10?'0':'')+sec;
      g('boostBtn').textContent=`Burnt Toast (${mm}:${ss})`;
      g('btinfo').textContent=` ${Math.round(100*br/bt)}% left`;
    } else {
      g('boostBtn').textContent=s.boost?'Burnt Toast (ON)':'Burnt Toast (OFF)';
      g('btinfo').textContent='';
    }
...
static void reflect_setpoint_ui(){
  ...
  if (ui_Setpoint_Slider && !slider_drag){
    // Best-effort mapping: assume slider in whole degrees
    ui_prog = true;
    lv_slider_set_value(ui_Setpoint_Slider, (int)roundf(setpoint_c), LV_ANIM_OFF);
    ui_prog = false;
  }
#endif
}
...
static void mqtt_loop_task(){
  ...
  if (mqtt.connect(MQTT_CLIENTID, mqtt_user.c_str(), mqtt_pass.c_str())){
    ...
    // publish retained state on connect
...
static void apply_power_off(){
  ...
  set_boost_button_enabled(false);
...
static void apply_power_on(){
  ...
  // Immediately give control back to AUTO using saved/retained setpoint
  auto_mode = true;
  manual_hold_until_ms = 0;
  auto_control_task(true);   // temps unknown => 30%
}
...
static void stop_burnt_toast(){
  ...
  if (mqtt.connected()){
    mqtt.publish(T_BOOST_ACTIVE,"OFF",true);
    mqtt.publish(T_BOOST_REMAIN_S,"0",true);
  }

  if (!power_off){
    set_slider_enabled(true);
    set_setpoint_enabled(true);
    // After Toast, hold manual 30% during the window to avoid fighting AUTO
    auto_mode = false;
    manual_hold_until_ms = millis() + MANUAL_HOLD_MS;
    target_percent = 30;
...
static inline void apply_new_setpoint(float v, bool do_beep){
  ...
  if (!power_off && !boost_active){
    auto_mode = true;
    manual_hold_until_ms = 0;
    auto_control_task(true);   // unknown temps => 30%
  }
}
#ifdef UI_HAS_SETPOINT
...
static void on_setpoint_slider_cb(lv_event_t* e){
  if (ui_prog) return;
  lv_event_code_t code = lv_event_get_code(e);
  lv_obj_t* obj = (lv_obj_t*)lv_event_get_target(e);
  int v_int = lv_slider_get_value(obj);
  float v = v_int;               // assume whole °C slider
  if (code == LV_EVENT_VALUE_CHANGED){
    if (ui_Setpoint_Value) ui_set_label_text(ui_Setpoint_Value, String(v,1));
  } else if (code == LV_EVENT_RELEASED){
    apply_new_setpoint(v, true);
  }
}
#endif

/***********************
 *  UI callbacks (fan/power/toast)
 ***********************/
...
static void handle_status() {
  ...
  String mac  = WiFi.isConnected() ? WiFi.macAddress() : WiFi.softAPmacAddress();
...
// MQTT config GET/SET (persist to NVS + trigger reconnect)
static void handle_mqtt_cfg(){
  bool changed = false;

  if (web.hasArg("host")) {
    String v = web.arg("host"); v.trim();
...
// Wi-Fi config GET/SET (persist to NVS + reconnect)
static void handle_wifi_cfg(){
  bool changed = false;

  if (web.hasArg("ssid")) {
    String v = web.arg("ssid"); v.trim();
    if (v.length()) { sta_ssid = v; prefs.putString(KEY_WIFI_SSID, sta_ssid); changed = true; }
  }
  if (web.hasArg("pass")) {
  ...
  if (changed){
    ...
    delay(100);
    ap_mode = false;
    wifi_begin_ms = 0;  // cause wifi_loop_task() to try STA begin() again quickly
    web.send(200,"application/json","{\"ok\":true}");
    return;
  }
...
/***********************
 *  WIFI (non-blocking; safe start for WebServer/mDNS)
 ***********************/
...
static void wifi_loop_task(){
  ...
  if (wifi_begin_ms == 0){
    WiFi.mode(WIFI_STA);
    WiFi.begin(sta_ssid.c_str(), sta_pass.c_str());    // non-blocking
...
void setup(){
...
#ifdef UI_HAS_SETPOINT
  // Hook Setpoint Up/Down buttons or Slider (rename IDs if yours differ)
  ...
  if (ui_Setpoint_Slider){
    lv_obj_add_event_cb(ui_Setpoint_Slider, on_setpoint_slider_cb, LV_EVENT_VALUE_CHANGED, NULL);
    lv_obj_add_event_cb(ui_Setpoint_Slider, on_setpoint_slider_cb, LV_EVENT_RELEASED,      NULL);
  }
#endif
  ...
  // Persistence: load last setpoint + WiFi + MQTT config
  prefs.begin(NVS_NS, false);
  setpoint_c = prefs.getFloat(KEY_SETPOINT, 18.0f);   // default 18C
  sta_ssid   = prefs.getString(KEY_WIFI_SSID, WIFI_SSID_DEF);
  sta_pass   = prefs.getString(KEY_WIFI_PASS, WIFI_PASSWORD_DEF);
...
void loop(){
  ...
  // Publish remaining Burnt Toast seconds over MQTT (retained)
  int br = boost_remaining_s();
  if (mqtt.connected() && (br != last_boost_pub_s)) {
